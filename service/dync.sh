...
exec="/usr/bin/dyncserver"
prog="dyncserver"
config="/etc/dyncserver.conf"
...
stop() {
    ...
    su $USER -c "dyncserver -d stop"
    retval=$?
    rm -f $lockfile
    if [ ! $retval -eq 0 ] ; then
        echo "Dyncserver with pid $pid could not be stopped."
        return 1
    fi
    return 0
}
